...
public:
  MyCustomOpSizeComputer() {
    MNN_PRINT("MyCustomOpSizeComputer()\n");
  }
...
#ifdef DEBUG
      MNN_PRINT("Error for binary op: input0's type != input1's type, %d != "
                "%d, optype:%d, ",
                input0->getType().code, input1->getType().code, opType);
      if (nullptr != op->name()) {
