...
ErrorCode XPUTensorConverter::convert(const void *inputRaw, void *outputRaw,
  ...
  if (source == dest) {
    if (tId == 0) {
      ::memcpy(outputRaw, inputRaw, batch * area * channel * byteNum);
    }
    return NO_ERROR;
  }
  if (MNN_DATA_FORMAT_NHWC == source && MNN_DATA_FORMAT_NCHW == dest) {
    if (tId == 0) {
      switch (byteNum) {
      case 1:
        NHWC2NCHW((int8_t *)inputRaw, (int8_t *)outputRaw, batch, channel,
                  area);
        break;
      case 2:
        NHWC2NCHW((int16_t *)inputRaw, (int16_t *)outputRaw, batch, channel,
                  area);
        break;
      case 4:
  ...
  if (MNN_DATA_FORMAT_NCHW == source && MNN_DATA_FORMAT_NHWC == dest) {
    if (tId == 0) {
      switch (byteNum) {
      case 1:
        NCHW2NHWC((int8_t *)inputRaw, (int8_t *)outputRaw, batch, channel,
                  area);
        break;
      case 2:
        NCHW2NHWC((int16_t *)inputRaw, (int16_t *)outputRaw, batch, channel,
                  area);
        break;
      case 4:
  ...
  // dest, inside, outside, channel);
  if (MNN_DATA_FORMAT_NC4HW4 == source) {
    if (1 == inside) {
      int offset[2] = {outside, outside};
      ...
      auto inputStart = (int8_t *)inputRaw + (start * core->pack * byteNum);
      auto outputStart = (int8_t *)outputRaw + (start * channel * byteNum);
      if (core->bytes == byteNum) {
      ...
      proc((float *)outputStart, (const float *)inputStart, end - start,
           channel, offset);
    } else {
      if (core->bytes == byteNum) {
      ...
      if (batch != 1) {
        // Divide in batch
        int offset[2] = {outside * inside, area};
        ...
        for (int v = start; v < end; ++v) {
          auto inputStart =
              (int8_t *)inputRaw + (v * core->pack * byteNum * area);
          auto outputStart =
              (int8_t *)outputRaw + (v * channel * byteNum * area);
          proc((float *)outputStart, (const float *)inputStart, area, channel,
               offset);
        }
      } else {
        // Divide in area
        int offset[2] = {area, area};
        ...
        auto inputStart = (int8_t *)inputRaw + (start * core->pack * byteNum);
        auto outputStart = (int8_t *)outputRaw + (start * byteNum);
        proc((float *)outputStart, (const float *)inputStart, end - start,
             channel, offset);
      }
    }
  ...
  if (MNN_DATA_FORMAT_NC4HW4 == dest) {
    if (1 == inside) {
      ...
      auto outputStart = (int8_t *)outputRaw + (start * core->pack * byteNum);
      auto inputStart = (int8_t *)inputRaw + (start * channel * byteNum);
      ...
      if (batch != 1) {
        // Divide in batch
        int offset[2] = {area, outside * inside};
        int step = UP_DIV(batch, numberThread);
        int start = tId * step;
        int end = std::min(start + step, batch);
        if (end <= start) {
        ...
        for (int v = start; v < end; ++v) {
          auto outputStart =
              (int8_t *)outputRaw + (v * core->pack * byteNum * area);
          auto inputStart =
              (int8_t *)inputRaw + (v * channel * byteNum * area);
          proc((float *)outputStart, (const float *)inputStart, area, channel,
               offset);
        }
      } else {
        ...
        auto outputStart =
            (int8_t *)outputRaw + (start * core->pack * byteNum);
        auto inputStart = (int8_t *)inputRaw + (start * byteNum);
        proc((float *)outputStart, (const float *)inputStart, end - start,
             channel, offset);
      }
    }
...
std::tuple<int, int, int> XPUTensorConverter::splitDimensions(const halide_buffer_t& ib, MNN_DATA_FORMAT source) {
...
ErrorCode XPUTensorConverter::convert(const Tensor* input, const Tensor* output, const CoreFunctions* core, int tId, int numberThread) {
    ...
    if (ib.dimensions <= 1 || source == dest) {
        ...
        // printf("convert # dataSize, byteNum = %d, %d\n", dataSize, byteNum);
        // fflush(stdout);
        ::memcpy(ob.host, ib.host, dataSize * byteNum);
        return NO_ERROR;
    }
