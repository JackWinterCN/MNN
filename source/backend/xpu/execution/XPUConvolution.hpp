...
#ifndef XPU_CONVOLUTION_HPP
#define XPU_CONVOLUTION_HPP
...
namespace MNN {
    class XPUPerfConfig {
public:
    XPUPerfConfig() : isParallelInner{false}, eTile{1}, ePack{1}, hPack{1}, instructionCosts{.0f} {
    }
    XPUPerfConfig(bool isParallelInner_, int eTile_, int ePack_, int hPack_, float instructionCosts_)
        : isParallelInner{isParallelInner_},  eTile{eTile_}, ePack{ePack_}, hPack{hPack_}, instructionCosts{instructionCosts_} {
    }
    bool operator!=(const XPUPerfConfig& other) {
        return isParallelInner != other.isParallelInner || ePack != other.ePack || eTile != other.eTile || hPack != other.hPack;
    }
    XPUPerfConfig& operator=(const XPUPerfConfig& other) {
...
class XPUConvolution : public Execution {
public:
    ...
    XPUConvolution(const Convolution2DCommon *convOp, Backend *b);
    virtual ~XPUConvolution() = default;
