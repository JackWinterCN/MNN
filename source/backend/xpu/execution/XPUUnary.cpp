...
#include "XPUUnary.hpp"
#include "XPUUnaryUtils.hpp"
...
#include "backend/xpu/execution/compute/XPUConvOpt.h"
...
XPUUnary::XPUUnary(Backend *b, MNNUnaryExecute proc, MNNUnaryExecuteInt8 procInt8, const Op* op) : MNN::Execution(b), mProc(proc), mProcInt8(procInt8){
    MNN_PRINT("UnaryOp Type: %d\n",op->main_as_UnaryOp()->opType());
...
ErrorCode XPUUnary::onResize(const std::vector<Tensor *> &inputs, const std::vector<Tensor *> &outputs) {
...
MNNUnaryExecute XPUUnary::selectForFloat(int type, int precision) {
    switch (type) {
...
MNNUnaryExecuteInt8 XPUUnary::selectForInt8(int type) {
#ifdef MNN_SUPPORT_QUANT_EXTEND
    switch (type) {
        case UnaryOpOperation_ABS:
            return _ABSInt8;
...
ErrorCode XPUUnary::onExecute(const std::vector<Tensor *> &inputs, const std::vector<Tensor *> &outputs) {
    auto input  = inputs[0];
    auto output = outputs[0];
    // auto size = static_cast<XPUBackend*>(backend())->getTensorSize(input);
    // auto schedule = ((XPUBackend*)backend())->multiThreadDivide(size);
    auto size = input->elementSize();
    std::pair<int, int> schedule = {size, 1};
    auto inputPtr = (uint8_t*)input->deviceId();
...
class XPUUnaryCreator : public XPUBackend::OpCreator {
public:
    virtual Execution *onCreate(const std::vector<Tensor *> &inputs, const std::vector<Tensor *> &outputs,
                                const MNN::Op *op, Backend *backend) const override {
        auto core = static_cast<XPUBackend*>(backend)->functions();
        auto precision = static_cast<XPUBackend*>(backend)->precisionMode();
        auto type = inputs[0]->getType();
        MNNUnaryExecute proc = nullptr;
        MNNUnaryExecuteInt8 procInt8 = nullptr;
#ifdef MNN_SUPPORT_QUANT_EXTEND
        if (getDataType(inputs[0]) == DataType_DT_INT8 || inputs[0]->getType().bytes() == 1) {
        ...
        if (nullptr == procInt8) {
            if (type.code == halide_type_int) {
                ...
                proc = XPUUnary::selectForFloat(op->main_as_UnaryOp()->opType(), static_cast<XPUBackend*>(backend)->precisionMode());
        ...
        return new XPUUnary(backend, proc, procInt8, op);
    }
};
