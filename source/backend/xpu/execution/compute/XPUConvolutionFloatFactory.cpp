//
//  XPUConvolutionFloatFactory.cpp
//  MNN
//
...
static Execution* _createUnit(const Tensor* input, const Tensor* output, Backend* backend,
    ...
    auto cpuBackend = (XPUBackend*)backend;
...
Execution* XPUConvolutionFloatFactory::create(const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs,
    ...
    bool lowMemory;
#ifdef MNN_LOW_MEMORY
    lowMemory = static_cast<XPUBackend*>(backend)->memoryMode() == BackendConfig::Memory_Low;
    // if (static_cast<XPUBackend*>(backend)->functions()->bytes == 2 && static_cast<XPUBackend*>(backend)->int8Functions()->MNNGemmInt8AddBiasScale_Unit_FP16 == nullptr) {
    //     // Fall back to fp32
    //     return nullptr;
    // }
#else
...
//     lowMemory = lowMemory || (static_cast<XPUBackend*>(backend)->memoryMode() != BackendConfig::Memory_High);
// #endif
