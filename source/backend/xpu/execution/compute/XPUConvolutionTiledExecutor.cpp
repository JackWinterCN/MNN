//
//  XPUConvolutionTiledExecutor.cpp
//  MNN
//
...
namespace MNN {
namespace XPU {
void XPUConvolutionTiledExecutor::initWeight(const float *source, float* cache, int depth, int outputCount, int kernelSize, const XPUCoreFunctions* function) {
    // Swap k, ic
    int dims[4] = {
        depth,
        kernelSize,
...
XPUConvolutionTiledExecutor::XPUConvolutionTiledExecutor(Backend* b, const float* bias, size_t biasSize)
    : MNN::Execution(b) {

    mResource.reset(new XPUConvolution::Resource);
    mResource->backend = b;
...
XPUConvolutionTiledExecutor::XPUConvolutionTiledExecutor(std::shared_ptr<XPUConvolution::Resource> res, Backend* b) : mResource(res), Execution(b) {
}

XPUConvolutionTiledExecutor::~XPUConvolutionTiledExecutor() {
    // Do nothing
}
bool XPUConvolutionTiledExecutor::onClone(Backend* bn, const Op* op, Execution** dst) {
    if (!mValid) {
        return false;
    }
    if (nullptr == dst) {
    ...
    *dst = new XPUConvolutionTiledExecutor(mResource, bn);
    return true;
}
...
std::pair<size_t, std::pair<size_t, size_t>> XPUConvolutionTiledExecutor::computeBlitInfoSize(int eP, int ow, int kernelSize, int threadNumber) {
...
std::pair<int, bool> XPUConvolutionTiledExecutor::turnIm2ColToBlitInfo(float const ** srcPtr, int32_t* el, int start, int xC, const ConvolutionCommon::Im2ColParameter& p, const uint8_t* srcOrigin, int bytes) {
