...
namespace MNN {
using namespace XPU;
void XPUConvolution::Resource::copyBias(float* dst, const float* bias, int outputCount, Backend* backend) {
...
bool XPUConvolution::Resource::copyBiasAlign(const float* bias, int outputCount) {
    auto core = static_cast<XPUBackend*>(backend)->functions();
    int bytes = core->bytes;
    int unit = core->pack;
    auto alignOutput = UP_DIV(outputCount, unit) * unit;
    ...
    copyBias((float*)mBias->deviceId(), bias, outputCount, backend);
    return true;
}
XPUConvolution::MutableResourceInt8::MutableResourceInt8(std::shared_ptr<ResourceInt8> res, Backend* backend, float* scalePtr) : mResource(res) {
...
void XPUConvolution::MutableResourceInt8::updateInputOutputScale(std::vector<float> inputQuantInfo, std::vector<float> outputQuantInfo) {
...
/*
std::shared_ptr<XPUConvolution::ResourceInt8> XPUConvolution::makeResourceInt8(Backend* backend, const MNN::Op* op, int pack) {
    auto convParam = op->main_as_Convolution2D();
    auto core = static_cast<XPUBackend*>(backend)->functions();
    // TODO: use different pack from float
    int UNIT = pack;

    std::shared_ptr<XPUConvolution::ResourceInt8> resource(new ResourceInt8);
...
XPUConvolution::XPUConvolution(const Convolution2DCommon *convOp, Backend *b) : MNN::Execution(b), mCommon(convOp) {
    // Do nothing
}
std::vector<float> XPUConvolution::getPostParameters() const {
...
int XPUConvolution::reorderWeightSize(int depth, int outputCount, int kernelSize, int unitDepth, int unitOC) {
    return UP_DIV(outputCount, unitOC) * UP_DIV(depth, unitDepth) * kernelSize * unitDepth * unitOC;
}



ErrorCode XPUConvolution::onResize(const std::vector<Tensor *> &inputs, const std::vector<Tensor *> &outputs) {
    auto input  = inputs[0];
...
class XPUConvolutionFactory : public XPUBackend::OpCreator {
public:
    virtual Execution *onCreate(const std::vector<Tensor *> &inputs, const std::vector<Tensor *> &outputs,
                                const MNN::Op *op, Backend *backend) const override {
        return XPUConvolutionFloatFactory::create(inputs, outputs, op, backend);
    }
};


// class XPUConvInt8Creator : public XPUBackend::Creator {
// public:
//     virtual Execution* onCreate(const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs,
//                                 const MNN::Op* op, Backend* backend) const override {
...
//         if (static_cast<XPUBackend*>(backend)->functions()->pack == 4 && convOp->sparseParameter() && SparseConvInt8TiledExecutor::shouldUseSparse(convOp)) {
//             auto res = XPUConvolution::makeResourceInt8(backend, op, core->pack);
...
REGISTER_XPU_OP_CUSTOMIZED_CREATOR(XPUConvolutionFactory, OpType_Convolution);
// REGISTER_XPU_OP_CUSTOMIZED_CREATOR(XPUConvInt8Creator, OpType_ConvInt8)
